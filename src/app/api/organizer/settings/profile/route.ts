...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const profileRes = await client.query(
      `SELECT company_name, bio, website, phone, logo_url FROM organizer_profiles WHERE user_id = $1 LIMIT 1`,
      [user.id]
    );
...
export async function POST(req: NextRequest) {
  ...
  const { name, companyName, bio, website, phone, avatarUrl } = body || {};
  ...
  try {
    ...
    await client.query(
      `INSERT INTO organizer_profiles (user_id, company_name, bio, website, phone, logo_url)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (user_id) DO UPDATE SET 
         ...
         updated_at = now()`,
      [userId, companyName || null, bio || null, website || null, phone || null, avatarUrl || null]
    );
